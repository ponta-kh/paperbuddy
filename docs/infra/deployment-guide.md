...
2. AWSへログイン済みであることを確認し、`infra/.env`を用意する
...
各端末でAWS CLIによるログインが完了していることを前提とする。AWS Profile、IAM Identity Centerなどの認証操作は各端末の責務とし、このリポジトリでは認証情報を管理しない。
...
デプロイ先リージョンとBedrock生成モデルは`infra/.env`で管理する。
