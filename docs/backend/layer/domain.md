...
## Docstring
Domain層では、業務ルールの意図、不変条件、状態遷移、Domain例外を読み手に伝えるためにdocstringを記述する。

- Entity、Value Object、Domain Service、Command Repository Protocolには原則としてdocstringを書く
- クラスdocstringには、そのドメイン概念が何を表すか、どのような不変条件を持つかを書く
- 公開メソッドには、業務上の意味がある場合にdocstringを書く
- Argsは、引数に業務上の意味や制約がある場合に書く
- Returnsは、戻り値の意味が自明でない場合に書く
- Raisesは、Domain例外、不変条件違反、業務ルール違反を表す例外がある場合に書く
- 小さな判定メソッドで、名前と戻り値型から意味が明らかな場合は、概要のみでもよい

docstringはGoogle Style Docstringを使用する。実装の細かな手順はコードで表現し、業務上または設計上の理由を補足する必要がある場合はインラインコメントを使用する。
