...
- 保証する整合性: `Chat.last_updated_at`の更新と新しい2メッセージを同一トランザクションで永続化する
...
| --- | --- | --- | --- |
| `ChatQueryRepositoryProtocol` | `get_chat_by_id_for_user` | 所有ユーザーに紐づく継続対象チャットを取得する | `RepositoryNotFoundError` |
...
| `ChatCommandRepositoryProtocol` | `save_exchange` | 更新済みチャット本体と新しいユーザー・LLMメッセージを同一トランザクションで永続化する | `ChatSaveError` |

## 9. 基本フロー

1. 入力されたプロンプトから`Prompt`を生成し、一般的な前後空白の除去と文字数制約を適用する。
2. 認証済みユーザーに紐づく指定チャットを取得する。
...
8. `Chat.record_exchange`で発信順序を検証し、最終更新日時を正常回答日時へ更新する。[DR-05@chat] [DR-07@chat]
9. 更新済みチャット本体と新しい2メッセージを同一トランザクションで保存する。
10. チャットID、AI回答、既存タイトルを返す。
...
flowchart TD
    ...
    B -->|有効| C["get_chat_by_id_for_userでチャットを取得"]
    ...
    F -->|有効| G["save_exchangeで同一トランザクション保存"]
...
| `RepositoryNotFoundError` | 指定チャットが存在しない、または認証済みユーザーが所有しない | 外部サービスを呼び出さず、永続化しない | 例外を送出する |
...
- 代替系: 該当なし
- 異常系: 対象なし、所有者不一致、継続期限超過、外部サービス利用不可、不正応答、保存失敗
