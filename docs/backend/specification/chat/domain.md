...
- チャットID、タイトル、所有ユーザーID、作成日時、最終更新日時を持つチャット
...
- 種別: Entity
- 識別子: `chat_id`。空文字および空白文字のみではない`str`
- 責務: ユーザーに紐づくチャットを表現し、チャットID、タイトル、所有ユーザーID、作成日時、最終更新日時の整合性を保証する

#### 属性

| 属性名 | 型 | 必須 | 説明・制約 |
| --- | --- | --- | --- |
| `chat_id` | `str` | 必須 | チャットを一意に識別する値。空文字および空白文字のみは不可 |
...
| 操作 | 入力 | 結果 | 関連するDomainルール | 送出するDomain例外 |
| --- | --- | --- | --- | --- |
| `create` | `chat_id`, `title`, `user_id`, `answered_at` | `created_at`と`last_updated_at`に初回の正常回答日時`answered_at`を設定した`Chat`を生成する | DR-01, DR-06 | `InvalidChatIdError` |
| `record_exchange` | ユーザーの`ChatMessage`、LLMの`ChatMessage` | 同じチャットターンに属することと継続会話の発信順序を検証し、`last_updated_at`をLLMメッセージの発信日時へ更新する | DR-02, DR-05, DR-07 | `MessageSentAtOutOfOrderError`, `InvalidMessageSenderError`, `InvalidChatTurnError` |
...
| 属性名 | 型 | 必須 | 説明・制約 |
| --- | --- | --- | --- |
| `chat_id` | `str` | 必須 | メッセージが属するチャットの識別子 |
| `turn_id` | `ChatTurnId` | 必須 | ユーザー質問と対応するLLM回答を関連付ける識別子 |
...
| 操作 | 入力 | 結果 | 関連するDomainルール | 送出するDomain例外 |
| --- | --- | --- | --- | --- |
| 生成 | `chat_id`, `turn_id`, `sender`, `content`, `sent_at` | `ChatMessage`を生成する | DR-01, DR-02 | `InvalidChatIdError`, `InvalidPromptError`, `PromptTooLongError`, `InvalidMessageSenderError` |
...
- 違反時のDomain例外: `InvalidPromptError`, `PromptTooLongError`
...
- 違反時のDomain例外: `InvalidMessageSenderError`

### ChatTurnId

- 表現する値: ユーザー質問と対応するLLM回答を1対1で関連付けるチャットターンの識別子
- 保持する値: UUID
- 等価性: 同じUUIDである場合に等価
- 制約: 有効なUUIDであること
- 振る舞い: 同一チャットターンに属する2件のメッセージを関連付ける
- 違反時のDomain例外: `InvalidChatTurnIdError`
...
- 対象: `Chat`
- 内容: `chat_id`は空文字および空白文字のみではない`str`でなければならない
- 違反時のDomain例外: `InvalidChatIdError`

### DR-02: チャットメッセージの構成

- 対象: `ChatMessage`
- 内容: チャットメッセージは、チャットID、チャットターンID、ユーザーまたはLLMの発信者、発信者に対応する内容、発信日時を必ず持つ。ユーザー発信の内容には`Prompt`を使用し、LLM発信の内容には1,000文字の上限を適用しない
- 違反時のDomain例外: `InvalidPromptError`, `PromptTooLongError`, `InvalidMessageSenderError`, `InvalidChatTurnIdError`
...
- 違反時のDomain例外: `MessageSentAtOutOfOrderError`

### DR-06: チャット日時の初期状態

- 対象: `Chat`
- 内容: チャット生成時の`created_at`と`last_updated_at`は、初回の正常なLLM回答日時と一致しなければならない
- 違反時のDomain例外: 該当なし。生成操作が同じ日時を設定する

### DR-07: チャットターンの構成

- 対象: ユーザーの`ChatMessage`、LLMの`ChatMessage`
- 内容: 1つのチャットターンは、同じ`chat_id`と`turn_id`を持つユーザーメッセージ1件とLLMメッセージ1件で構成する
- 違反時のDomain例外: `InvalidChatTurnError`
...
| --- | --- | --- |
| `InvalidChatIdError` | `chat_id`が空文字または空白文字のみ | 不正な識別子を持つチャットの生成を拒否するため |
...
| `InvalidChatTurnIdError` | チャットターンIDが有効なUUIDではない | 質問と回答を関連付けられないメッセージの生成を拒否するため |
| `InvalidChatTurnError` | 同一ターンにユーザー質問とLLM回答が1件ずつ存在しない、または`chat_id`・`turn_id`が一致しない | 不完全または不整合な会話の保存を拒否するため |
...
Repositoryは、同一トランザクションで整合性を保証する保存単位に対応した操作を定義する。

| Protocol | 操作 | 引数 | 戻り値 | 送出する例外 |
| --- | --- | --- | --- | --- |
| `ChatCommandRepositoryProtocol` | `save_started_chat` | `Chat`, 同じ`ChatTurnId`を持つユーザー・LLMの`ChatMessage` | なし | `ChatSaveError` |
| `ChatCommandRepositoryProtocol` | `save_exchange` | 更新済み`Chat`, 同じ`ChatTurnId`を持つユーザー・LLMの`ChatMessage` | なし | `ChatSaveError` |
...
- 正常系: UUIDのユーザーIDを持つ有効なチャットの生成、作成日時と最終更新日時の一致、同じチャットターンIDを持つユーザー質問とLLM回答、有効なプロンプトの生成と整形、時系列に沿った会話記録
- 境界値: 整形後のプロンプトが空文字、1文字、999文字、1,000文字、1,001文字となる場合
- 異常系: 一般的な空白文字のみのプロンプト、1,000文字を超えるユーザープロンプト、不正なチャットID、不正な発信者、不正なチャットターンID、質問と回答の`chat_id`・`turn_id`不一致、最新メッセージより過去の発信日時
- トランザクション: 初回開始時は`Chat`本体と初回2メッセージ、会話継続時は`Chat.last_updated_at`更新と新規2メッセージが同時に保存されること
...
- チャットIDの重複保存時に必要なRepository契約例外
- 同時刻に複数の会話継続処理が実行された場合の競合制御
