...
| 操作 | 入力 | 結果 | 関連するDomainルール | 送出するDomainエラー |
| --- | --- | --- | --- | --- |
| 生成 | `chat_id`, `turn_id`, `sender`, `content`, `sent_at` | `ChatMessage`を生成する | DR-01, DR-02 | `InvalidChatIdError`, `InvalidPromptError`, `PromptTooLongError`, `InvalidMessageSenderError`, `InvalidChatTurnError` |
