...
| `ensure_owned_by` | `user_id` | 指定ユーザーがチャット所有者であることを確認する | DR-11 | `ChatOwnershipMismatchError` |
| `ensure_continuable_at` | `requested_at` | `requested_at`時点で会話を継続できることを確認する | DR-10 | `ChatContinuationExpiredError` |
...
- 対象: `Chat`
- 内容: チャットは`last_updated_at`から継続要求日時までが24時間以内の場合のみ継続できる。24時間を超過した時点から継続不可とする
- 違反時のDomainエラー: `ChatContinuationExpiredError`
...
| `ChatOwnershipMismatchError` | 指定ユーザーがチャット所有者ではない | 所有者以外による会話継続を拒否するため |
...
| --- | --- | --- | --- | --- |
| `ChatCommandRepositoryProtocol` | `get_chat` | `chat_id` | 対象の`Chat` | `ChatNotFoundError`, `ChatLoadError` |
...
- 正常系: UUID v7のチャットIDとリクエストID、有効なユーザーID、有効なセッションIDを持つチャットの生成、作成日時と最終更新日時の一致、同じリクエストIDを持つユーザー質問とLLM回答、有効なプロンプトの生成と整形、時系列に沿った会話記録、所有者による会話継続、24時間以内の会話継続
- 境界値: 整形後のプロンプトが空文字、1,000文字、1,001文字となる場合、および最終更新から24時間、24時間1秒の会話継続
- 異常系: 空文字または空白文字のみのセッションID、一般的な空白文字のみのプロンプト、1,000文字を超えるユーザープロンプト、不正な発信者、不正なリクエストID、質問と回答の`chat_id`・`request_id`不一致、最新メッセージより過去の発信日時、所有者不一致、24時間を超過した会話継続
