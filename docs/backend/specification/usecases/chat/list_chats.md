...
1. `ChatQueryRepositoryProtocol.list_chats_by_user_id`へユーザーIDを渡す。
...
```mermaid
flowchart TD
    A([開始]) --> B["list_chats_by_user_idでチャット一覧を取得"]
    B -->|対象なし| C["空の一覧へ変換"]
...
- 発生条件: `ChatQueryRepositoryProtocol`が`RepositoryNotFoundError`を送出する
- 振る舞い: 空の一覧へ変換する
- 結果: 正常終了し、空の一覧を返す
...
該当なし。ユーザーIDによる取得条件と並び順は出力ポート契約で保証する。
