...
- チャット生成結果として検証済みのセッションID、AI回答を取得する
...
8. 採番したチャットID、セッションID、タイトル、初回の正常回答日時を基準として、`created_at`と`last_updated_at`が一致する`Chat`を生成する。
...
flowchart TD
    ...
    D -->|検証済み応答| TITLE["整形済みプロンプトからタイトルを決定"]
    TITLE --> ID["UUID v7のchat_idを採番"]
    ID --> T["初回の正常回答日時を記録"]
    T --> E["chat_id・session_id・タイトルを持つChat本体と\n同じchat_id・request_idを持つ初回2メッセージを生成\n[DR-01@chat] [DR-06@chat] [DR-07@chat] [DR-08@chat]"]
...
| `InvalidChatGenerationResponseError` | セッションID、AI回答を有効な応答として扱えない | 永続化しない。チャット生成サービス側で作成されたチャットは取り消せない | 例外を送出する |
...
- 引用情報はAI回答の付帯情報として呼び出し元へ返し、LLMメッセージに紐づけて永続化する。
...
- チャット生成結果が有効なセッションID、AI回答を含まない場合は異常終了し、永続化されない
...
- 境界値: TRIM後の空文字、タイトル省略なしの10文字、タイトル省略ありの11文字、1,000文字、1,001文字
