...
description: バックエンドの外部接続仕様書を、Infrastructure層と出力ポートの契約に整合させて docs/backend/specification/integrations/[integration名].md へ新規作成する。外部API、LLM、ストレージ、SDK、DBなどの接続方式、入出力変換、例外変換、設定、制約を実装前に明文化する場合に使用する。
---
...
- `docs/backend/specification/integrations/[integration名].md` に作成する。
