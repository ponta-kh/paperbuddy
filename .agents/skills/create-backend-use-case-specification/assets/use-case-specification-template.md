...
Protocol名、操作名、契約例外名は、Application層またはDomain層が必要とする能力と失敗理由を表す抽象的な名称とする。特定の外部サービス、製品、SDK、DBなどの実装名を含めない。
...
Domain例外、Application例外、出力ポート契約上の例外と、ユースケースとしての振る舞いを記載する。HTTPステータスコード、公開エラーコード、SDK固有例外など、依存範囲外の情報は記載しない。
