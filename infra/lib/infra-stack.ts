...
export class InfraStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: InfraStackProps) {
    ...
    const knowledgeBaseId = new cdk.CfnParameter(this, 'BedrockKnowledgeBaseId', {
      type: 'String',
      ...
      'BackendService',
      {
        ...
        taskImageOptions: {
          image: ecs.ContainerImage.fromAsset(
            props.backendAssetPath ?? path.join(__dirname, '../../backend'),
          ),
          containerPort: 8000,
          environment: {
    ...
    backendService.taskDefinition.taskRole.addToPrincipalPolicy(
      new iam.PolicyStatement({
        ...
        resources: [chatTable.tableArn, `${chatTable.tableArn}/index/gsi1`],
      }),
    );
    backendService.taskDefinition.taskRole.addToPrincipalPolicy(
      new iam.PolicyStatement({
    ...
    new s3Deployment.BucketDeployment(this, 'DeployFrontend', {
      ...
      sources: [
        s3Deployment.Source.asset(
          props.frontendAssetPath ?? path.join(__dirname, '../../frontend/dist'),
        ),
      ],
      distribution,
      distributionPaths: ['/*'],
